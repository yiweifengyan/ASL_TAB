...
template <typename T>
std::vector<T> DirectPad(T* x, int padding1, int padding2, int N, int C, int H, int W) {
        ...
        for (int ic = 0; ic < C; ic++) {
            for (int ih = 0; ih < H; ih++) {
                for (int iw = 0; iw < W; iw++) {
                    qxptr[((in * C + ic) * packH + (ih + padding1)) * packW + iw + padding2] = x[((in * C + ic) * H + ih) * W + iw];
                }
            }
        }
    }
...
std::vector<float> DirectConv2d_FP32(float* x, float* w, int stride1, int stride2, int N, int C, int H, int W, int KN, int KH, int KW) {
    const int OH = H - KH + 1;
    const int OW = W - KW + 1;
    ...
    for (int n = 0; n < N; n++) {
        for (int c = 0; c < C; c++) {
            for (int h = 0; h < H; h++) {
                for (int w = 0; w < W; w++) {
                    // Use N_C_H_W format
                    T xx = X[((n * H + h) * W + w) * C + c] - X2[((n * H + h) * W + w) * C + c];
                    // std::cout<<X[((n * C + c) * H + h) * W + w] <<" X2: "<< X2[((n * C + c) * H + h) * W + w]<<std::endl;
                    if ((xx > 0.01) || (xx < -0.01)) {
                        std::cout << "n: " << n << ", h: " << h << ", w: " << w << ", c: " << c ;
                        std::cout << ", X1: " << X[((n * H + h) * W + w) * C + c] << ", X2: " << X2[((n * H + h) * W + w) * C + c] << std::endl;
                        return -1;
                    }
                }
            }
...
// Direct Compare Implemented in FP
template <typename T>
int Compare_Tensor_BNN_Padding(T* X, T* X2, int N, int C, int H, int W, int PaddingH, int PaddingW) {
    for (int n = 0; n < N; n++) {
        for (int c = 0; c < C; c++) {
